...
class PyInstallerTab:
    ...
    def create_basic_tab(self):
        ...
        self.script_entry = ctk.CTkEntry(
            script_input_frame, placeholder_text="选择要打包的Python脚本"
        )
        ...
        self.name_entry = ctk.CTkEntry(
            name_frame, placeholder_text="打包后的应用名称（可选）"
        )
        ...
        console_check = ctk.CTkCheckBox(
            console_options_frame, text="显示控制台窗口", variable=self.console_var
        )
        ...
        self.output_entry = ctk.CTkEntry(
            output_input_frame, placeholder_text="打包后应用的输出目录（默认：./dist）"
        )
        ...
        self.icon_entry = ctk.CTkEntry(
            icon_input_frame, placeholder_text="应用图标文件（可选）"
        )
    ...
    def create_advanced_tab(self):
        ...
        self.work_entry = ctk.CTkEntry(
            work_input_frame, placeholder_text="临时工作文件存放目录（默认：./build）"
        )
        ...
        for i, level in enumerate(log_levels):
            log_radio = ctk.CTkRadioButton(
                log_options_frame, text=level, variable=self.log_var, value=level
            )
        ...
        yes_check = ctk.CTkCheckBox(
            options_frame, text="自动确认覆盖非空输出目录", variable=self.yes_var
        )
    ...
    def create_files_tab(self):
        ...
        self.hidden_entry = ctk.CTkEntry(
            hidden_input_frame, placeholder_text="输入要隐藏导入的模块名"
        )
        ...
        self.exclude_entry = ctk.CTkEntry(
            exclude_input_frame, placeholder_text="输入要排除的模块名"
        )
        ...
        self.exclude_listbox = tk.Listbox(
            exclude_frame, height=6, font=self.listbox_font
        )
        ...
        exclude_remove_btn = self.create_remove_button(
            exclude_list_frame, "移除选中", self.remove_exclude_module
        )
        exclude_remove_btn.pack(side="left", padx=10, pady=5)

        exclude_clear_btn = self.create_clear_button(
            exclude_list_frame, "清空全部", self.clear_exclude_modules
        )
    ...
    def create_build_tab(self):
        ...
        self.build_btn = self.create_primary_button(
            build_btn_frame, "开始打包", self.start_build, height=40
        )
