...
class NotificationTestApp:
    ...
    def create_widgets(self):
        ...
        title_label = ctk.CTkLabel(
            self.root, text="通知系统测试", font=ctk.CTkFont(size=24, weight="bold")
        )
        ...
        self.message_entry = ctk.CTkEntry(
            input_frame, placeholder_text="输入自定义通知消息...", height=30
        )
    ...
    def test_warning_notification(self):
        ...
        custom_message = self.message_entry.get().strip()
        message = (
            custom_message if custom_message else "文件已存在，继续操作将覆盖原有内容。"
        )
