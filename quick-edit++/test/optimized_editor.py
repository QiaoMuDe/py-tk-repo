...
class OptimizedModernEditorApp:
    def __init__(self):
        # 主窗口
        self.root = ctk.CTk()
        self.root.title("QuickEdit++ - 优化版编辑器")
        self.root.geometry("1000x700")

        # 添加标志，防止在拖动过程中频繁更新
        self.is_dragging = False
        self.last_sash_pos = 0
    ...
    def create_main_content(self, parent):
        ...
        # 优化事件绑定：只在拖动开始和结束时处理
        self.paned_window.bind("<ButtonPress-1>", self.on_sash_press)
    ...
    def on_sash_press(self, event):
        ...
        self.last_sash_pos = self.paned_window.sashpos(0)

    def on_sash_drag(self, event):
        """分割线拖动事件 - 只更新位置，不做其他处理"""
        # 在拖动过程中不做任何处理，只记录位置
        pass
    ...
    def create_file_tree_panel(self):
        ...
        # 文件树内容区域 - 使用原生tkinter TreeView提高性能
        tree_frame = ctk.CTkFrame(left_main_frame)
        tree_frame.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)

        # 创建一个简单的文件树模拟 - 使用原生tkinter TreeView
        self.create_optimized_file_tree(tree_frame)

        # 添加到PanedWindow
        self.paned_window.add(left_main_frame, weight=1)

    def create_optimized_file_tree(self, parent):
        """创建优化的文件树 - 使用原生tkinter TreeView"""
        ...
        self.style.configure(
            ...
            font=("Microsoft YaHei", 20),  # 设置字体和大小
            rowheight=40,
        )  # 设置更大的行高，确保20号字体能完全显示
        self.style.configure(
            "Treeview.Heading",
            background="#333333",
            foreground="white",
    ...
    def create_editor_panel(self):
        ...
        # 添加一些示例文本
        sample_text = """# QuickEdit++ 优化版编辑器

这是一个使用 CustomTkinter 和 ttk.PanedWindow 构建的优化版文本编辑器界面。
...
2. **文件树优化**
   - 使用原生tkinter TreeView替代嵌套的CTK组件
   - 减少UI组件数量
   - 提高渲染性能
