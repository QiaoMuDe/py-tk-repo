...
class SyntaxHighlighter:
    ...
    def __init__(self, app):
        ...
        # 防抖和事件优先级相关状态
        self._highlight_job_id = None  # 当前延迟高亮任务的ID
        self._last_highlight_time = 0  # 上次实际执行高亮的时间
        self._force_highlight_threshold = 500  # 强制执行高亮的阈值（毫秒）
    ...
    def _bind_events(self):
        ...
        if self.render_visible_only:
            ...
            # Linux系统的滚轮事件
            self.text_widget.bind("<Button-4>", self._handle_event, add="+")
            self.text_widget.bind("<Button-5>", self._handle_event, add="+")
        else:
            # 全部渲染模式 - 不需要绑定实时更新事件
    ...
    def _get_event_priority(self, event_type):
        ...
        priority_map = {
            ...
            # 滚动事件：中等响应（100ms）
            "<MouseWheel>": 100,
            "<Button-4>": 100,  # Linux滚轮上
            "<Button-5>": 100,  # Linux滚轮下
        ...
        return priority_map.get(event_type, 100)

    def _schedule_highlight(self, highlight_func, delay_ms):
        """
        改进的防抖机制：取消之前的任务，安排新的高亮任务
        确保在高频事件下也能执行高亮
        
        Args:
            highlight_func: 要执行的高亮函数
            delay_ms: 延迟时间（毫秒）
        """
        current_time = time.time() * 1000  # 转换为毫秒
        
        # 取消之前的未执行高亮任务
        if self._highlight_job_id:
            self.text_widget.after_cancel(self._highlight_job_id)
            self._highlight_job_id = None
        
        # 检查是否需要强制执行高亮（防止高亮任务被不断取消）
        time_since_last_highlight = current_time - self._last_highlight_time
        if time_since_last_highlight > self._force_highlight_threshold:
            # 如果距离上次高亮执行时间超过阈值，则立即执行
            delay_ms = min(delay_ms, 50)  # 最多延迟50ms
        
        # 安排新的高亮任务
        self._highlight_job_id = self.text_widget.after(
            delay_ms, 
            self._execute_highlight_safely(highlight_func)
        )

    def _execute_highlight_safely(self, highlight_func):
        """
        安全执行高亮，处理异常
        ...
        def wrapper():
            try:
                self._highlight_job_id = None
                self._last_highlight_time = time.time() * 1000  # 记录实际执行时间
                highlight_func()
    ...
    def _handle_event(self, event=None):
        ...
        # 获取事件类型并确定基础延迟
        event_type = event.type if event else "<<Modified>>"
        base_delay = self._get_event_priority(event_type)
        
        # 使用防抖机制安排高亮任务
        self._schedule_highlight(self.highlight, base_delay)
    ...
    def cleanup(self):
        ...
        try:
            ...
            if self._highlight_job_id:
                self.text_widget.after_cancel(self._highlight_job_id)
                self._highlight_job_id = None
            self._last_highlight_time = 0  # 重置上次高亮时间
        except Exception:
            pass
