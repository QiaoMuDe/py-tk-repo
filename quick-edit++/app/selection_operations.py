...
class SelectionOperations:
    ...
    def base64_encode(self):
        ...
        try:
            ...
            # 对选中文本进行Base64编码
            encoded_text = base64.b64encode(selected_text.encode('utf-8')).decode('utf-8')
    ...
    def base64_decode(self):
        ...
        try:
            ...
            # 对选中文本进行Base64解码
            try:
                decoded_text = base64.b64decode(selected_text.encode('utf-8')).decode('utf-8')
            except Exception:
                # 如果解码失败, 尝试忽略无效字符
                decoded_text = base64.b64decode(selected_text.encode('utf-8'), validate=False).decode('utf-8')
            ...
            messagebox.showerror("错误", f"Base64解码时出错: {str(e)}\n请确保选中的文本是有效的Base64编码")
    ...
    def camel_to_pascal(self):
        ...
        try:
            ...
            # 转换逻辑
            pascal_case = selected_text[0].upper() + selected_text[1:] if selected_text else ""
    ...
    def pascal_to_camel(self):
        ...
        try:
            ...
            # 转换逻辑
            camel_case = selected_text[0].lower() + selected_text[1:] if selected_text else ""
    ...
    def to_private_variable(self):
        ...
        try:
            ...
            # 转换逻辑
            private_var = "_" + selected_text if not selected_text.startswith('_') else selected_text
    ...
    def to_class_name(self):
        ...
        try:
            ...
            import re
            # 先将各种格式转换为帕斯卡
            pascal_case = re.sub(r'(?<!^)(?=[A-Z])', '_', selected_text).replace('_', ' ').replace('-', ' ').title().replace(' ', '')
            # 添加Class后缀
    ...
    def to_function_name(self):
        ...
        try:
            ...
            import re
            # 先将各种格式转换为驼峰
            snake_case = re.sub(r'(?<!^)(?=[A-Z])', '_', selected_text).lower().replace('-', '_')
    ...
    def compress_json(self):
        ...
        try:
            ...
            try:
                json_data = json.loads(selected_text)
                # 压缩JSON, 不添加任何缩进
                compressed_json = json.dumps(json_data, separators=(',', ':'), ensure_ascii=False)
            except json.JSONDecodeError as e:
                messagebox.showerror("JSON解析错误", f"无效的JSON格式: {str(e)}")
                return
    ...
    def _parse_csv_line(self, line):
        ...
        while i < line_length:
            ...
            if char == '"':
                ...
                # 处理引号外的空格
                # 检查是否是字段间的空格（用于自动补逗号）
                if current_field and i + 1 < line_length and line[i + 1] != ',' and line[i + 1] != ' ':
                    # 可能是字段间的空格，先保留，稍后处理
                    current_field += char
                    i += 1
                    continue
                elif not current_field and i + 1 < line_length and line[i + 1] != ',' and line[i + 1] != ' ':
                    # 可能是字段间的空格，先保留，稍后处理
                    current_field += char
                    i += 1
                    continue
    ...
    def format_python(self):
        ...
        try:
            ...
            try:
                ...
                messagebox.showerror("Black库缺失", "未安装black库，无法格式化Python代码")
                return
            except SyntaxError as e:
                messagebox.showerror("Python语法错误", f"无效的Python代码: {str(e)}")
                return
            except Exception as e:
                messagebox.showerror("Python格式化错误", f"格式化Python代码时出错: {str(e)}")
                return
    ...
    def format_yaml(self):
        ...
        try:
            ...
            try:
                ...
                messagebox.showerror("YAML库缺失", "未安装ruamel.yaml库，无法格式化YAML")
                return
            except Exception as e:
