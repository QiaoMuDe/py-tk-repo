...
class FindDialog:
    def __init__(self, parent, text_widget, file_path=None):
        ...
        # 初始化大文件查找器
        self.large_file_searcher = (
            LargeFileSearcher(file_path, text_widget) if file_path else None
        )

        # 创建对话框
        self.dialog = tk.Toplevel(parent)
        self.dialog.title("查找和替换")
        self.dialog.geometry("500x400")  # 调整大小以适应新的布局
    ...
    def create_widgets(self):
        ...
        # 查找选项框架
        options_frame = ttk.LabelFrame(main_frame, text="查找选项", padding="5")
        options_frame.grid(
        ...
        ).grid(row=0, column=0, sticky=tk.W, padx=(5, 0))
        ttk.Radiobutton(
        ...
        ).grid(row=1, column=0, sticky=tk.W, pady=(5, 0), padx=(5, 0))
        ttk.Checkbutton(
            options_frame, text="跨行匹配", variable=self.multiline_match
        ).grid(row=1, column=1, sticky=tk.W, pady=(5, 0), padx=(5, 0))

        # 进度条框架
        progress_frame = ttk.LabelFrame(main_frame, text="查找进度", padding="5")
        progress_frame.grid(
        ...
        button_frame.grid(row=6, column=0, columnspan=2, pady=(5, 0))
        ...
        self.find_button = ttk.Button(
            button_frame, text="查找下一个", command=self.find_next
        )
        self.find_button.grid(row=0, column=0, padx=(0, 2), sticky=tk.W + tk.E)

        self.find_prev_button = ttk.Button(
            button_frame, text="查找上一个", command=self.find_previous
        )
        self.find_prev_button.grid(row=0, column=1, padx=(2, 2), sticky=tk.W + tk.E)

        self.find_all_button = ttk.Button(
            button_frame, text="查找全部", command=self.find_all
        )
        self.find_all_button.grid(row=0, column=2, padx=(2, 2), sticky=tk.W + tk.E)

        # 取消按钮和关闭按钮
        self.cancel_button = ttk.Button(
            button_frame,
            text="取消",
            command=self.cancel_search_operation,
            state=tk.DISABLED,
        )
        self.cancel_button.grid(row=0, column=3, padx=(2, 2), sticky=tk.W + tk.E)

        self.close_button = ttk.Button(
            button_frame, text="关闭", command=self.dialog.destroy
        )
        self.close_button.grid(row=0, column=4, padx=(2, 0), sticky=tk.W + tk.E)

        # 第二行按钮 - 替换相关
        self.replace_once_button = ttk.Button(
            button_frame, text="替换一次", command=self.replace_once
        )
        self.replace_once_button.grid(
            row=1, column=0, padx=(0, 2), pady=(5, 0), sticky=tk.W + tk.E
        )

        self.replace_all_button = ttk.Button(
            button_frame, text="替换全部", command=self.replace_all
        )
        self.replace_all_button.grid(
            row=1, column=1, padx=(2, 2), pady=(5, 0), sticky=tk.W + tk.E
        )

        self.replace_prev_button = ttk.Button(
            button_frame, text="替换上一个", command=self.replace_previous
        )
        self.replace_prev_button.grid(
            row=1, column=2, padx=(2, 2), pady=(5, 0), sticky=tk.W + tk.E
        )

        self.replace_next_button = ttk.Button(
            button_frame, text="替换下一个", command=self.replace_next
        )
        self.replace_next_button.grid(
            row=1, column=3, padx=(2, 2), pady=(5, 0), sticky=tk.W + tk.E
        )

        # 占位按钮，使界面布局更加对称
        placeholder_button = ttk.Button(
            button_frame, text="暂无功能", state=tk.DISABLED
        )
        placeholder_button.grid(
            row=1, column=4, padx=(2, 0), pady=(5, 0), sticky=tk.W + tk.E
        )
    ...
    def cancel_search_operation(self):
        """取消当前查找操作"""
        self.cancel_search = True
        if self.large_file_searcher:
            self.large_file_searcher.cancel_search_operation()
        ...
        self.progress_label.config(text="查找已取消")
    ...
    def replace_next(self):
        ...
        if self.matches and 0 <= self.current_match_index < len(self.matches):
            ...
            messagebox.showinfo("替换", "已替换下一个匹配项")
    ...
    def replace_previous(self):
        ...
        if self.matches and 0 <= self.current_match_index < len(self.matches):
            ...
            messagebox.showinfo("替换", "已替换上一个匹配项")
    ...
    def perform_search(self, search_term):
        ...
        self.progress_label.config(text="正在查找...")
        self.progress_var.set(0)

        # 如果有大文件搜索器且文件路径有效，使用大文件搜索
        if hasattr(self, "large_file_searcher") and self.large_file_searcher:
            search_type = self.search_type.get()
            match_case = self.match_case.get()
            whole_word = search_type == "whole"

            # 根据查找类型决定使用哪种搜索方法
            if search_type == "regex":
                matches = self.large_file_searcher.search_in_file(
                    search_term, "regex", match_case, whole_word
                )
            else:
                matches = self.large_file_searcher.search_in_file(
                    search_term, "normal", match_case, whole_word
                )
            ...
            if not self.cancel_search:
                self.progress_label.config(
                    text=f"查找完成，找到 {len(matches)} 个匹配项"
                )
                ...
                if search_type == "regex":
                    ...
                    try:
                        ...
                        for match in compiled_pattern.finditer(content):
                            ...
                            # 更新进度
                            processed_length = match.end()
                            if total_length > 0:
                                progress = min(
                                    100, (processed_length / total_length) * 100
                                )
                        ...
                        self.find_all_button.config(state=tk.NORMAL)
                        self.cancel_button.config(state=tk.DISABLED)
                        self.progress_label.config(text="查找出错")
                        self.progress_var.set(0)
                        return []
                else:
                    # 普通查找或完整匹配
                    ...
                    while True:
                        ...
                        # 更新进度
                        try:
                            processed_length = int(start_pos.split(".")[0]) * 100 + int(
                                start_pos.split(".")[1]
                            )
                            if total_length > 0:
                                progress = min(
            ...
            if theme is None and hasattr(
                self.text_widget.master.master, "theme_manager"
            ):
    ...
    def highlight_all_matches(self):
        ...
        # 如果有大文件搜索器，使用它的高亮方法
        if hasattr(self, "large_file_searcher") and self.large_file_searcher:
            self.large_file_searcher.highlight_matches_in_view()
        else:
            # 清除之前的高亮
            self.text_widget.tag_remove("found", "1.0", tk.END)
            self.text_widget.tag_remove("current_match", "1.0", tk.END)
